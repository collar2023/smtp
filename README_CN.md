...
> [!NOTE]
> **模拟与生产环境的差异**：该 SMTP 状态机主要作为协议模拟层与教学演示使用（通过 `/api/smtp/feed` 接口进行会话）。在真实的实战环境下，网页端 SPA 和 CLI 客户端为了极致的简单、高效和鲁棒性，会绕过繁琐的交互式 SMTP 状态机，直接使用**无状态的 HTTPS API**（例如 `/api/send` 和 `/api/inbox`）进行密件的存取。
